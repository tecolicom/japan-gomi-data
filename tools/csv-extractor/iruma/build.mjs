...
for (const { region, bunbetsu, iso } of rows) {
  if (!fySet.has(iso)) throw new Error(`収録期間 ${PERIOD} 範囲外の日付: ${iso} (region=${region})`);
...
for (const [region, R] of regions) {
  ...
  writeFileSync(join(OUT, PERIOD, `course-${courseNo}.yaml`), header + yamlStringify(doc, { lineWidth: 0 }));
...
const taxonomy = {
  ...
  groups: [
    { label: 'ビン・缶・ペットボトル・有害ごみ', short: 'ビン缶等',
      members: ['glass_bottle', 'beverage_can', 'pet_bottle', 'hazardous'] },
  ],
};
const taxHeader =
  ...
  '# groups は参考情報: 市区の括り名 (同日一括回収の呼称)。データ本体は構成カテゴリへ分解済み。\n';
