...
function buildCourse(n) {
  ...
  const fyDates = periodDates(PERIOD);

  // カレンダーの収録範囲が収録期間と一致するか確認
...
for (const n of DISTRICTS) {
  ...
  writeFileSync(join(OUTDIR, PERIOD, `course-${n}.yaml`), header + courseYaml(n, built));
...
const meta = {
  ...
  notes: [
    ...
    '確率的信頼度: 上記は同一ソースからの再展開一致 (自己照合) であり、保証されるのはパースの忠実性。独立な誤り単位で見ると、規則単位の系統誤りは 4地区×9種別=36 パターン ゼロ不一致 → 95% 信頼で <8.3%/パターン、monthly_specific の実日付転記誤りは 1460 日枠 ゼロ不一致 → <0.21%/日枠 (rule of three)。ソース自体の独立照合は無いが、ソースは全戸配布カレンダーの原文そのもの (住民が従う基準と一致)。地区割当のみ日本郵便町域一覧と独立照合済み。',
  ],
};
const metaHeader = '';
