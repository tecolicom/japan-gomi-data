...
```
   ...
   **ただし出現回数で難易度を決めない。** 数えるのは「0 件かどうか」であって、
   少なければ難物という意味ではない。**曜日規則を見出しに 1 回だけ書く形式**がある。

   | survey の difficulty | 実際 |
   |---|---|
   | 足立 3 | 1,141 字あるのに「資源」2 件・「可燃」1 件で弱く見えたが、`資源 週1回【金】` のように**見出しに 1 回ずつ**あるだけだった。板橋と同型の良質なソース |
   | 大田 4 | 「xls」の字面で難物に見えたが、実体は `.xlsx` (XML の zip) で素直に読める町×曜日表 |

   0 件なら図版を疑う。1〜数件なら**開いて構造を見る**。
