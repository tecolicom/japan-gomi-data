...
4. `municipalities/<pref>/<handle>/survey.yaml` があれば読む。
   **survey の記述を鵜呑みにしない。** 誤りは 4 件見つかっている。
   ...
   | 豊島「多言語版は町名別曜日表を**含まず**」 | 2 ページ目が曜日表そのもの。しかも 6 言語版が別々に組版されていて版どうしの照合に使えた |

   共通するのは**見た目や文字数で判断して中身を確かめていない**こと。
   国分寺の「11,433 字」も数字自体は正しく、中身が日付と電話番号だった。
   豊島は「約17.4万字」と書かれていたが実測 4.8 万字で、その字数の正体が曜日表だった。
   **「◯◯が取れる」と書いてあったら、その語で実際に grep して件数を数える。**
   **「◯◯を含まない」も同じく数える** — 否定の記述は調べた形跡が残らないぶん危ない。
   豊島は survey が一次ソース無しと判定していたので、鵜呑みにすれば撤退していた。
