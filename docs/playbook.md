...
6. **テキスト層のない PDF** — 最終手段だが、**諦める前に `pdfplumber` で `lines` / `curves` / `rects` を数える**。
   罫線がベクタで生きていればグリッドは座標復元でき、品目が図版でも**塗り色**で判定できる (秩父広域・青梅)。
   「pdftotext で曜日が取れない = 画像 PDF」と決めつけないこと (青梅はこれで difficulty 4 と誤判定されていた)。
   本当に描画情報が無い場合 (Print To PDF 製・スキャン画像) だけ、全数転記はせず §3 のサンプリングで扱う。
