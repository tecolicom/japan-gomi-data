...
### 東京都カタログ / 中野区 (`tokyo/tokyo-nakano`, 収録準備中)

- 東京都オープンデータカタログ: https://catalog.data.metro.tokyo.lg.jp/
  - **CLI (curl/WebFetch) は CloudFront に 403 で弾かれる**。ページ閲覧・CSV ダウンロードはブラウザで行うこと (CKAN API も同様に弾かれた, 2026-07-16)。
- データセット: 中野区「資源とごみの収集曜日一覧」
  ...
  - ライセンス: カタログページで要確認 (東京都カタログの通例は CC BY 4.0。CLI 遮断のため未確認)。
