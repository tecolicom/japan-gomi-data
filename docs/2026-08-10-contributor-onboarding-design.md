...
3. `make regen HANDLE=<既存>` が全収録済み自治体で差分ゼロを返す
   ...
   → **スキップ区分がもう 1 つあった。「2026-08-15 追記」を見よ**
...
## 2026-08-15 追記 — 生成器の無い自治体という穴

飯能・日高の作業で分かったこと。**設計の想定に無い抜け道が実在し、そこを誤りが通った。**

### 何が起きたか

`make regen` のスキップ区分として想定していたのは「cache が無い」だけだったが、実際には
**「生成器そのものが無い」自治体**があった。データだけがあり、course YAML のヘッダは
`Auto-generated by tools/pdf-extractor` と名乗っているのに、その生成器が存在しない。
`scripts/check-regen.mjs` はこれを skip するので、**受け入れ基準 3 を満たしていても
検査されない自治体が残る**。

飯能はこの状態で、A-1 コースの読み違い (第 5 週にペットボトルと紙・布を出す規則) を
**4 か月ぶん配信していた**。人が PDF を目視するまで誰も気づけなかった。機械が照合する
経路が無かったからで、データや検査の質ではなく**検査の対象から外れていたこと**が原因である。

2026-08-15 時点で `make regen` は **検査 30 / skip 3 / 失敗 0** (収録済み 33)。
残る skip は sabae・higashichichibu-vill (生成器なし) と kamifurano-town
(生成器はあるが build.mjs 規約に乗っていない)。

### 設計への含意

- **受け入れ基準 3 に「skip がゼロであること」を加えるべき**。差分ゼロと検査ゼロは違う。
  現状の出力はスキップ理由を区別して出すので (「生成器が無い」「build.mjs が無い」「cache が無い」)、
  そのまま基準にできる
- 収録の完了条件に「生成器が残っていること」を含める。ヘッダの自称は当てにならない
- 手順は `docs/playbook.md` §6「既存収録の再構築」に置いた。既存データが照合の基準になるので
  新規収録とは順序が違う (extract → 全日付照合 → 差分の裁定 → build)

### 第 2 段を実装する人へ

**第 2 段 (schema と生成物) は 2026-08-15 時点で未着手**である。`source.provenance` は schema に
無く、`source.confidence` は残ったまま、`meta.summary` と `scripts/build-coverage.mjs` /
`docs/coverage.md` も無い。

追従対象の生成器が 2 本増えている。`provenance` / `summary` を投入するときは
`tools/pdf-extractor/hanno` と `tools/pdf-extractor/hidaka` も含めること。
どちらも course YAML だけを生成し、meta / taxonomy / facts / survey は手書きが正典。
