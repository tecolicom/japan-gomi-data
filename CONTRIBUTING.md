...
**差分が出たときは作業ツリーに再生成結果が残ります。** `make regen` は build を実際に
走らせて比較するため、失敗した自治体のファイルは書き換わったままです。中身を確認したら
`git checkout -- municipalities/<県>/<handle>/` で戻してください
(残したまま作業を続けると、無関係な差分を抱えたままコミットすることになります)。
