...
- 板橋区 — `itabashi` (24コース、59町134町字) ※区公式「地域別 資源回収・ごみ収集曜日カレンダー(令和8年度版)」の地域別PDF 24本 (東①〜⑫・西①〜⑫) 由来。**同じ日程が2通りに公表されていて両方が機械可読** — PDFはテキスト層に曜日規則を持ちつつ本体が3色の塗りセルによる通年カレンダー、索引ページのHTML表70行が「町丁目→地域」と曜日規則を持つ。地区割はPDFヘッダ「地域→町丁目」と**双方向照合**して不一致ゼロ、日付レベルも8,664日枠で不一致ゼロ。**規則が15通りしかなく9組が重複するがfoldCourseしない** (区が別運用・清掃事務所の管轄も東西で分かれる)。年末年始は区が確定を留保しているため `unknown_periods`。上赤塚町・下赤塚町はABRに現存するが区の日程資料に一度も現れないため未収録
- 江東区 — `koto` (12コース、46町158町字) ※区配布シート「令和8年度版 地区別 資源回収・ごみ収集日一覧」の**多言語版PDF**由来 — 日本語版は`chars` 0の画像PDF・裏面はCIDフォント化けで、テキスト層を持つのは中国語版/韓国語版だけ (豊島と同じ構図)。**都カタログCSV (CC BY 4.0) は地区割の正典にできない** — 現行版シートと豊洲・塩浜で逆方向に食い違い、CKANのlast_modifiedが2024-11でシートが令和8年度版のためシートを採った (中野のOD 2021年止まりと同型)。日本語の町名はCSVからのみ取れるので町名=CSV/地区割=シート。燃やさないごみは隔週でCSVに起点日が無いが、シートが12か月ぶんの実日付を印字しており`monthly_specific`で収録。年末年始は`unknown_periods` — 窓は隔週日付の抜けから**データで確定**した
- 中央区 — `chuo` (13コース、37町104町字) ※区公式「あなたの町のごみ・資源収集曜日」HTML表 (3地域56行) 由来。**照合は「ごみと資源の分け方・出し方」パンフレットの外国語版** — 日本語版は画像PDFだが英語版・中国語版には同じ曜日表がテキスト層つきで入っている (豊島・江東に続く3例目)。粗大ごみは申込制で収録しないが照合には使用。**繁華街 (京橋・銀座・日本橋1〜3丁目・八重洲) は燃やすごみが週6日**、日本橋人形町2丁目1〜14番だけ週5日。区が日本橋地域の町名から「日本橋」を省いているため正式町名へ戻した (19町33行)。**期間は `2026-01--2026-11`** — 年末年始の記述が公表資料に無いため12月を落とし、始端はパンフの発行月 (令和8年1月) に取った
