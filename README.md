...
- 三鷹市 — `mitaka` (10地区、11町62町字) ※市公式「収集日程」の地区別HTML (曜日規則の表) 由来。ABR の62町字を10地区へ重複・欠落なく分割できることを build が検査する。**年末年始はHTMLに無く**地区別カレンダーPDFから読んだ: 12/29(火)は火曜が可燃の5地区だけ臨時収集、1/1(金)休業で金曜に第n枠を持つ2地区が1週繰り下がり第5金曜にも収集がある。PDFは品目マークが画像でテキストにならないため、休止日と注記は座標から機械抽出し、本体は2地区の12月・1月を全日88日枠 目視転記して照合
- 豊島区 — `toshima` (22コース、20町83丁目→102area) ※区公式**外国語版リーフレット**「資源回収・ごみ収集のお知らせ」と「曜日一覧」の2ページ目由来 — 町丁名も曜日も日本語のまま印字され訳が右に並ぶ。同じ表を**6言語版が別々に組版**しているので版どうしの照合ができる (5版×58行)。保存版冊子の曜日一覧は `chars` 0 の画像PDFのため58行を目視転記し、収録期間の全日24,888日枠で照合して不一致ゼロ。**期間は `2026-04--2026-11`** — 祝日は通年通常収集だが年末年始の休止日が未公表のため12月を落とした (12月の告知後に延ばす)。サーベイの app-only / difficulty 4 は誤りで、pdf-text / 2 へ訂正した
